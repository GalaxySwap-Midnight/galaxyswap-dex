 ...
 */
export async function requestFaucetTokens(address: string, captchaToken: string): Promise<boolean> {
  const endpoints = [
    '/api/request-tokens',
    '/request',
    '/api/request'
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      console.log(`[Faucet] Failed with endpoint: ${endpoint}, status: ${response.status}, error: ${errorData}`);
  ...
  throw new Error('All faucet endpoints failed. Please check the faucet service status.');
} 