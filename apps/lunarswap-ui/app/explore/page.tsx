...
export default function ExplorePage() {
  ...
  const tokens = allPairs.length > 0 ? getAvailableTokensForSelection(allPairs) : Object.values(popularTokens);
              ...
              <table className="w-full">
                <thead>
                  <tr className="text-left text-sm text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="pb-3 pt-4 px-6 font-medium">Token</th>
                    <th className="pb-3 pt-4 px-6 font-medium">Name</th>
                    <th className="pb-3 pt-4 px-6 font-medium">Type</th>
                    <th className="pb-3 pt-4 px-6 font-medium">Address</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTokens.map((token, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-6">
                        <button
                          type="button"
                          onClick={() => copyToClipboard(token.symbol, `token-${token.symbol}`)}
                          className="flex items-center space-x-3 w-full text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors rounded p-2 -m-2 cursor-pointer"
                        >
                      ...
                      <td className="py-4 px-6">
                        <button
                          type="button"
                          onClick={() => copyToClipboard(token.name, `name-${token.symbol}`)}
                          className="w-full text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors rounded p-2 -m-2 cursor-pointer text-gray-600 dark:text-gray-400"
                        >
                          {token.name}
                        </button>
                      </td>
                      <td className="py-4 px-6">
                        <button
                          ...
                          className="w-full text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors rounded p-2 -m-2 cursor-pointer font-mono text-sm text-gray-500 dark:text-gray-400"
                        >
                          {token.type}
                        </button>
                      </td>
  ...
  const renderPoolsContent = () => {
    ...
    const filteredPools = allPairs ? 
      Array.from(new Map(allPairs.map(pool => [pool.pairId, pool])).values())
        .filter(pool => {
          ...
          return token0Symbol.toLowerCase().includes(searchLower) || 
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">
                          {poolSearchQuery ? 'No Pools Found' : 'No Pools Available'}
                        </h3>
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 px-6">
                              <button
                                type="button"
                                onClick={() => navigate(`/explore/pool/${pool.pairId}`)}
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500 dark:text-gray-400">
                                    {pool.pairId.slice(0, 8)}...{pool.pairId.slice(-8)}
                                  </div>
                                </div>
                              </button>
                            </td>
              ...
              {exploreOptions.map((option) => {
                ...
                return (
                  <Button
                    ...
                    size="sm"
                    disabled
                    className="px-4 py-2 rounded-full text-gray-400 dark:text-gray-500 opacity-60"
                  >
                    {option.title}
